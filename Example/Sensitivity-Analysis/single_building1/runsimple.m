...
% each time step.
logInput = zeros(MAXSTEPS, 3);
logOutput = zeros(MAXSTEPS, 18);
...
%     end
%     
%     if tro1 > coosp && tro1 < heasp && cooling == false
%         ThSch = 0;
%     end
    
    inputs = [coosp heasp ThSch];
...
% Plot results
subplot(3,1,1),plot(time, Tout, 'k', time, Tro1, 'g',  time, Tro2, 'c',  time, Tro3, 'm',  time, Tro4, 'r--', time, Tro5, 'b--', time, Tcoo, 'b', time, Thea, 'r', time, Fro5, 'c', time, Ther, 'm');
legend('Outdoor', 'Troom1', 'Troom2', 'Troom3', 'Troom4', 'Troom5', 'Cool SP', 'Heat SP', 'Air Flow');
title('Temperatures');
xlabel('Time (hour)');
ylabel('Temperature (C)');
grid on;

subplot(3,1,2),plot(time, Ptot, 'r', time, Pfan, 'k', time, Phpu, 'g', time, Pcpu, 'b', time, Pchi, 'm');
...
subplot(3,1,3),plot(time, Tpin, 'r', time, Tpou, 'b', time, Tout, 'k');
...
save('data_24_200kW_20m3_1000m.mat', 'time', 'Tout', 'Tro3', 'Tro4', 'Tcoo', 'Thea', 'Fro3', 'Fro4', 'Ptot', 'Pfan', 'Phpu', 'Pcpu','Pchi', 'Tpin', 'Tpou');
