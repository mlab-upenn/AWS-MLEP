function data = readIDFAWS(filename, classnames)
...
if noClassnames
    ...
    data = repmat(struct('class', '', 'fields', {{}}, 'start', {{}}, 'finish', {{}}), 1, 128);
else
    data = struct('class', classnames, ...
        'fields', repmat({{}}, 1, nClassnames), ...
        'start',  repmat({{}}, 1, nClassnames), ...
        'finish',  repmat({{}}, 1, nClassnames));
end
...
while true
    ...
    for k = 1:length(toks)
        if ~inBlock
            ...
            if noClassnames
                ...
                if saveBlock
                    ...
                    data(classIdx).start{end+1} = lineNumber;
                    %data(classIdx).finish{end+1} = {};
        ...
        if toks{k}{2} == ';'
            inBlock = false;
            data(classIdx).finish{end+1} = lineNumber;
        end
    end
end
